               ...
               const uint32_t fors_addr[8])
{
    ...
    uint32_t fors_tree_addr_base[8] = {0};
    uint32_t fors_pk_addr[8] = {0};
    unsigned int i;
    copy_keypair_addr(fors_tree_addr_base, fors_addr);
    copy_keypair_addr(fors_pk_addr, fors_addr);
    set_type(fors_tree_addr_base, SPX_ADDR_TYPE_FORSTREE);
    ...
    for (i = 0; i < SPX_FORS_TREES; i++) {
        ...
        memcpy(fors_tree_addr, fors_tree_addr_base, sizeof(fors_tree_addr));
        unsigned char *sig_local = sig + i * SPX_N * (1 + SPX_FORS_HEIGHT);
        uint32_t idx_offset = i * (1 << SPX_FORS_HEIGHT);
        set_tree_height(fors_tree_addr, 0);
        set_tree_index(fors_tree_addr, indices[i] + idx_offset);
        fors_gen_sk(sig_local, sk_seed, fors_tree_addr);
        sig_local += SPX_N;
        treehash(roots + i*SPX_N, sig_local, sk_seed, pub_seed,
