...
/**
 * Workload-aware parallel version of treehash: caps the thread count to
 * the number of leaves, avoiding unnecessary thread pool overhead when
 * the workload is smaller than the available core count.
 */
void treehash_parallel(unsigned char *root, unsigned char *auth_path,
              const unsigned char *sk_seed, const unsigned char *pub_seed,
              uint32_t leaf_idx, uint32_t idx_offset, uint32_t tree_height,
              void (*gen_leaf)(
                 unsigned char* /* leaf */,
              ...
              uint32_t tree_addr[8])
{
    ...
    uint32_t num_leaves = (uint32_t)(1 << tree_height);
    unsigned char leaves[num_leaves * SPX_N];


    /* Phase 1: generate all leaves in parallel, using the OpenMP default thread count
       without manually limiting the thread count. */
#pragma omp parallel for schedule(static)
    for (idx = 0; idx < num_leaves; idx++) {
        uint32_t local_addr[8];
        memcpy(local_addr, tree_addr, sizeof(local_addr));
        gen_leaf(leaves + idx*SPX_N, sk_seed, pub_seed, idx + idx_offset, local_addr);
    }

    /* Phase 2: original serial stack-based tree merge, reading precomputed leaves. */
    for (idx = 0; idx < num_leaves; idx++) {
        memcpy(stack + offset*SPX_N, leaves + idx*SPX_N, SPX_N);
        offset++;
        heights[offset - 1] = 0;
        if ((leaf_idx ^ 0x1) == idx) {
            memcpy(auth_path, stack + (offset - 1)*SPX_N, SPX_N);
