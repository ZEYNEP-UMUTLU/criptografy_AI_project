 ...
 */
unsigned long long spx_128f_secretkeybytes(void)
{
 ...
 */
unsigned long long spx_128f_publickeybytes(void)
{
 ...
 */
unsigned long long spx_128f_bytes(void)
{
    return CRYPTO_BYTES;
}
 ...
 */
unsigned long long spx_128f_seedbytes(void)
{
 ...
 */
int spx_128f_seed_keypair(unsigned char *pk, unsigned char *sk,
                             const unsigned char *seed)
{
    ...
    treehash_parallel(sk + 3*SPX_N, auth_path, sk, sk + 2*SPX_N, 0, 0, SPX_TREE_HEIGHT,
 ...
 * Format pk: [PUB_SEED || root]
 */
int spx_128f_keypair(unsigned char *pk, unsigned char *sk)
{
  unsigned char seed[CRYPTO_SEEDBYTES];
  randombytes(seed, CRYPTO_SEEDBYTES);
  spx_128f_seed_keypair(pk, sk, seed);
 ...
 */
int spx_128f_signature(uint8_t *sig, size_t *siglen,
                          const uint8_t *m, size_t mlen, const uint8_t *sk)
{
    ...
    for (i = 0; i < SPX_D; i++) {
        ...
        treehash_parallel(root, sig, sk_seed, pub_seed, idx_leaf, 0,
 ...
 */
int spx_128f_verify(const uint8_t *sig, size_t siglen,
 ...
 */
int spx_128f_sign(unsigned char *sm, unsigned long long *smlen,
                const unsigned char *m, unsigned long long mlen,
                const unsigned char *sk)
{
    size_t siglen;

    spx_128f_signature(sm, &siglen, m, (size_t)mlen, sk);
 ...
 */
int spx_128f_open(unsigned char *m, unsigned long long *mlen,
                     ...
                     const unsigned char *pk)
{
    ...
    *mlen = smlen - SPX_BYTES;
    ...
    if (spx_128f_verify(sm, SPX_BYTES, sm + SPX_BYTES, (size_t)*mlen, pk)) {
        memset(m, 0, smlen);
        *mlen = 0;
        return -1;
    }
